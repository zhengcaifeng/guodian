(function(a){
	a.fn.gallerySlide=function(k){
		...
		var n=p.pics.length;
		var o=function(){
			var q="<ul style='position:absolute;top:0;left:0;'><li><a href='"+p.pics[0].href+"' target='"+p.pics[0].target+"'><img src='"+p.pics[0].src+"' width='"+p.width+"' height='"+p.height+"' /></a></li></ul>";
			i.css({
				position:"relative"
			}).html(q);
			i.find("ul").css({
		...
		var j=function(){
			...
			i.bind("mouseover",function(){
				$("#gallery .o-control").show()
			}).bind("mouseleave",function(){
				$("#gallery .o-control").hide()
			})
		};
		var l=function(r,q){
			if(r==n){
