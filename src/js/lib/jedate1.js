...
(function (win) {
    ...
    jeDt.nowDate = function (timestamp, format) {
        ...
        }(parseInt(timestamp,10)) : +new Date());
    ...
    var config = {
        ...
        maxDate: "9999-12-31 23:59:59", //最大日期
    ...
    InitDate.prototype = {
        init: function () {
            ...
            //} else {
            //注释掉判断，修复动态添加多个元素时，Chrome和IE浏览器中除了最后一个元素外其他元素无法动态绑定BUG
        ...
        setHtml: function (opts, self) {
            ...
            var bothmsStr = !isformat ? '<ul class="botflex jedatehms"><li><em data-hms="24"></em><i>:</i></li><li><em data-hms="60"></em><i>:</i></li><li><em data-hms="60"></em></li></ul>' + '<div class="botflex jedatebtn"><span class="jedateok" style="width:50px;">确认</span><span class="jedatetodaymonth" style="width:50px;display:none;">今天</span><span class="jedateclear" style="width:50px;">清空</span></div>' : '<div class="botflex jedatebtn"><span class="jedateok" style="width:50px;">确认</span><span class="jedatetodaymonth" style="width:50px;">本月</span><span class="jedateclear" style="width:50px;">清空</span></div>';
            ...
            if (opts.isTime) {
                var dhmsArr = jeDt.val(self) != "" || jeDt.text(self) != "" ? [Tms[3], Tms[4], Tms[5]] : ['00', '00', '00'];
        ...
        onlyYMevents: function (that, opts, self, Tms) {
            var ymPre = QD(Cell + " .jedateym .ymprev")[0], ymNext = QD(Cell + " .jedateym .ymnext")[0], ony = parseInt(Tms[0],10), onm = parseInt(Tms[1],10);
        ...
        getDateStr: function (y, m, d) {
            ...
            //先得到当前月第一天是星期几.
            var date = setMonthDays(y, m), weekday = new Date(y, parseInt(m,10) - 1, 1).getDay();
            //根据这个星期算前面几天的上个月最后几天.
            var pervLastDay = weekday != 0 ? weekday : weekday + 7;
            //得到上个月最后一天;
            var pervMonthlastDay = getPervMonthLastDay(y, m), currentMonthDays = getPervMonthLastDay(y, parseInt(m,10) + 1);
            //上月最后几天循环
            ...
            var startDay = 1,
                ...
                firstDate = new Date(y, m - 1, 1),
                lastDate = new Date(y, m - 1, currentMonthDays),
                minTime = new Date(minArr[0], minArr[1] - 1, minArr[2]),
                maxTime = new Date(maxArr[0], maxArr[1] - 1, maxArr[2]),
                minDateDay = minTime.getDate();
            if (minTime > lastDate) {
                startDay = parseInt(currentMonthDays,10) + 1;
            ...
            for (var p = pervLastDay - 1; p >= 0; p--) {
                ...
                m == 1 ? (py = parseInt(y,10) - 1, pm = 13) : (py = y, pm = m);
                var thatpretm = parseInt(py.toString() + jeDt.digit(parseInt(pm,10) - 1,10).toString() + preDays.toString()), minpretm = parseInt(minArr[0].toString() + jeDt.digit(minArr[1]).toString() + jeDt.digit(minArr[2]).toString(),10), maxnexttm = parseInt(maxArr[0].toString() + jeDt.digit(maxArr[1]).toString() + jeDt.digit(maxArr[2]).toString(),10);
                preCls = thatpretm >= minpretm && thatpretm <= maxnexttm ? "prevdate" : "disabled";
                dayStr += "<li class='" + preCls + "' data-y='" + py + "' data-m='" + (parseInt(pm,10) - 1) + "' data-d='" + preDays + "'>" + isfestival(parseInt(pm,10) - 1 + "." + preDays, preDays) + mks(py, (parseInt(pm,10) - 1), preDays) + "</li>";
            }
            ...
            for (var n = 1; n <= nextMonthStartDays; n++) {
                var ny, nm, nextCls;
                n = jeDt.digit(n);
                m >= 12 ? (ny = parseInt(y,10) + 1, nm = 0) : (ny = y, nm = m);//niulei 2016.9.5 修改parseInt(nm,"10")
                var thatnexttm = parseInt(ny.toString() + jeDt.digit(parseInt(nm,10) + 1,10).toString() + jeDt.digit(n).toString()), minnexttm = parseInt(minArr[0].toString() + jeDt.digit(minArr[1]).toString() + jeDt.digit(minArr[2]).toString(),10), maxnexttm = parseInt(maxArr[0].toString() + jeDt.digit(maxArr[1]).toString() + jeDt.digit(maxArr[2]).toString(),10);
                nextCls = thatnexttm <= maxnexttm && thatnexttm >= minnexttm ? "nextdate" : nextCls = "disabled";
                dayStr += "<li class='" + nextCls + "' data-y='" + ny + "' data-m='" + (parseInt(nm,10) + 1) + "' data-d='" + n + "'>" + isfestival(parseInt(nm,10) + 1 + "." + n, n) + mks(ny, (parseInt(nm,10) + 1), n) + "</li>";
            }
            jeDt.html(QD(Cell + " .jedaul")[0], dayStr);
            jeDt.attr(QD(Cell + " .monthprev")[0], "data-y", jeDt.digit(parseInt(m,10) - 1));
            jeDt.attr(QD(Cell + " .monthnext")[0], "data-y", jeDt.digit(parseInt(m,10) + 1));
        ...
        events: function (that, opts, self, Tms) {
            ...
            if (!isformat) {
                ...
                jeDt.each([yPre, yNext], function (i, cls) {
                    jeDt.on(cls, "click", function (ev) {
                        jeDt.stopmp(ev);
                        var y = parseInt(jeDt.attr(jedateyear, "data-year"),10), m = parseInt(jeDt.attr(jedatemonth, "data-month"),10);
                        cls == yPre ? y -= 1 : y += 1;
            ...
            jeDt.on(QD(Cell + " .jedatebot .jedateok")[0], "click", function (ev) {
                ...
                if (!isformat) {
                    var okTimeArr = [], okTime = [parseInt(jeDt.attr(jedateyear, "data-year"),10), parseInt(jeDt.attr(jedatemonth, "data-month"),10), Tms[2]];
                    ...
                    //修复直接点击“确认”按钮，不选中当前日期Bug
                    var okVal = jeDt.parse([okTime[0], okTime[1], okTime[2]], [okTimeArr[0], okTimeArr[1], okTimeArr[2]], opts.format);
                    ...
                    //修复直接点击“确认”按钮，不选中当前日期Bug
                    var jedYM = jeDt.attr(QD(Cell + " .jedaym .action")[0], "data-onym").match(/\w+|d+/g),
                        okVal = jeDt.parse([jedYM[0], jedYM[1], 1], [0, 0, 0], opts.format);
        ...
        chooseYM: function (that, opts, self, Tms) {
            ...
            function clickLiYears(year) {
                jeDt.each(QD(Cell + " .ymdropul li"), function (i, cls) {
                    jeDt.on(cls, "click", function (ev) {
                        ...
                        var Years = jeDt.attr(_this, "data-y"), Months = jeDt.attr(jedatemonth, "data-month");
            ...
            jeDt.on(jedateyy, "click", function () {
                var YMlen = parseInt(jeDt.attr(jedateyy, "data-ym"),10), yearAttr = parseInt(jeDt.attr(jedateyear, "data-year"),10);
                setYearMonth(yearAttr, YMlen);
                clickLiYears(jedateyear);
            });
            //下拉选择月
            jeDt.on(jedatemm, "click", function () {
                var YMlen = parseInt(jeDt.attr(jedatemm, "data-ym"),10), yearAttr = parseInt(jeDt.attr(jedateyear, "data-year"),10);
                setYearMonth(yearAttr, YMlen);
                jeDt.each(QD(Cell + " .ymdropul li"), function (i, cls) {
                    jeDt.on(cls, "click", function (ev) {
                        ev = ev || window.event;
                        var _this = ev.srcElement || ev.target;
                        var Years = jeDt.attr(jedateyear, "data-year"), Months = jeDt.attr(_this, "data-m");
            ...
            var yearMch = parseInt(jeDt.attr(jedateyear, "data-year"),10);
        ...
        chooseDays: function (that, opts, self) {
            jeDt.each(QD(Cell + " .jedaul li"), function (i, cls) {
                jeDt.on(cls, "click", function (ev) {
                    ...
                    var aty = parseInt(jeDt.attr(cls, "data-y"),10) | 0, atm = parseInt(jeDt.attr(cls, "data-m"),10) | 0, atd = parseInt(jeDt.attr(cls, "data-d"),10) | 0;
        ...
        //扩展三个方法，重置日期(最大值和最小值)为初始状态，设置最小日期，设置最大日期
        //重置最大日期和最小日期为初始状态
        resetDate: function () {
            this.config.maxDate = '2099-12-31 23:59:59';
            this.config.minDate = '1900-01-01 00:00:00';
        },
        //设置最小日期
